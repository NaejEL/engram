...
| **M-13** | **Scinder `V-div`** ; **fusion déconseillée** (un FAIL sans cause nommée est un FAIL qu'on discute après coup). | §4.7. Sous décision AUC, `V-puissance` rend **K ≥ 429** ⇒ FAIL à tout N ≤ 80 : **la porte déclare elle-même l'indécidabilité**. |
...
- **Puissance** : `K_S ≥ (1.96·σ₀/|θ−T|)²`. Sous `R1_36`, σ₀ = 0.5 (Bernoulli, conservateur), |θ−T| = 0.25 ⇒ **K ≥ 15.37 ⇒ K ≥ 16**. Sous décision **AUC** à T = 0.9622 : **K ≥ 429** ⇒ **FAIL à tout N ≤ 80** — la porte **déclare elle-même l'indécidabilité**.
...
| --- | --- | --- |
| **N — nulle** | IC 95 % de **`ΔR1`** (différence appariée, argmax re-sélectionné) **contient 0** | **pas d'identité d'unité mesurable sous cosinus.** Les 36 leurres ne sont pas à construire en l'état. **C'est le résultat que la gate existe pour produire.** |
