...
### Positions du PI sur les avis Math / Neuro du tour matériau (2026-08-23, **post-avis**)

À verser à la consolidation. **Rien ne part au banc avant l'instruction complète du point 4.**

| # | Point | Position du PI |
| --- | --- | --- |
| **1** | **Partition `N-a` / `N-b` / `N-ind`** | **ACCEPTÉE telle que dérivée par Math.** *« Un test à seuil zéro sur quantité bruitée n'était pas une frontière, c'était un tirage au sort une fois sur deux à `K = 20`. »* Trois exigences d'exécution : **(a)** les **trois** classes au banc, **un cas synthétique par classe, y compris `N-ind`** (D18 : la totalité de la fonction est un item de couverture) ; **(b)** le verdict gravé de `N-ind` est **« indécidable ICI, matériel ou `K` insuffisant »**, *jamais un demi-`N-a`* — sa consigne de suite est **« augmenter la résolution »**, pas « conclure prudemment » ; **(c)** l'interdit symétrique de Math (**aucun agrégat binomial sur les 240 indicateurs**, échangeabilité **par requête** seulement) se grave en **clause de PORTÉE** au pré-enregistrement, **même famille que la portée de la clause NaN (D23)** — *« c'est le 0-23 dans les deux sens, fermé une fois pour toutes »*. |
| **2** | **Porte de troncature au point de capture** | **ACCEPTÉE et GÉNÉRALISÉE en D24-b.** Le défaut de Neuro (adjoint postposé ⇒ variantes **bit-identiques à la capture** ⇒ `R1_ident = 1` **par arithmétique**) est *« le mode de mort D24 sous une forme que D24 ne voit pas »*. Forme générale gravée : **toute nulle ET toute condition de contraste publient leur cardinal de séquences distinctes sur le PRÉFIXE CAUSALEMENT VISIBLE à la quantité mesurée**, jamais sur séquences complètes — *« en LM causal, ce qui suit la capture n'existe pas pour l'état capturé ; tout cardinal calculé sur la séquence entière est une **fiction** »*. **Conséquence de design à vérifier à la génération** : tout contraste décisionnel (variante, type, identité) doit être **réalisé AVANT le point de capture dans les trois cadres**, au banc comme **propriété du matériau** (D25 : garantie vérifiée, pas convention d'usage). |
| **3** | **Bin dur** | **DESCRIPTIF avant mesure**, conformément à Math. *« ~23 familles requises, `N` atteignable ~16-20 : l'indécidabilité est **arithmétique**. Le déclarer descriptif MAINTENANT est la leçon 0-13 appliquée d'avance — une porte qui déclare l'indécidabilité vaut mieux qu'une clause qui la découvre. »* Ligne à graver : *« le bin dur ne peut entrer en décision qu'à ≥ 23 familles ; à ce `N` il est descriptif, et aucune formulation du rapport ne le cite comme évidence »* — **le vocabulaire interdit du §2 s'étend à lui**. |
| **4** | **Budget** — conflit 48-sans-marge / 40-avec-marge | **Le conflit est RÉEL et la piste 4 × 5 × 3 = 60 le dissout — SI elle survit à l'instruction.** *« Ma position (marge > résolution) était fondée sur l'histoire des budgets du projet ; l'arithmétique de Neuro (2,5 familles/domaine impossible, S-5 violé) est **plus forte qu'une préférence** — **un design arithmétiquement impossible ne se préfère pas**. »* Trois questions d'instruction, **à trancher AVANT adoption** : **(a)** les 3 paires internes par famille sont-elles **échangeables** au sens de la nulle `1/37` de Math, ou le partage intra-famille crée-t-il une **strate de similarité** exigeant sa propre nulle et son cardinal D24-b ? **(b)** le passage de 2 à 3 membres change-t-il **les quatre conditions de construction** sous lesquelles Math a démontré l'exactitude de la nulle ? **Les re-dériver, pas les supposer stables** *(la leçon des acquis de régime recopiés)*. **(c)** **coût de génération réel des 60 unités sous TOUTES les contraintes cumulées** (longueur intra-type constante, contraste avant capture, BPE testé, type capitalisé pour S-7) — **chiffré avant adoption, pas découvert à la génération**. **Repli si (a) ou (b) échoue** : 48 / `K = 16` **avec amendement** — générer **4 familles surnuméraires de réserve**, qualifiées au **même banc**, substituables **UNIQUEMENT avant le premier token du run** (D14) : *la marge déplacée dans le **matériau** plutôt que dans `K`*. |
| **5** | **Annulation de la prédiction signée de Neuro** | **Correcte sur le fond** (le design en supprime le fondement) **et doit être CONSIGNÉE avec son motif au journal, pas silencieusement retirée** : *« une prédiction signée qui disparaît sans trace est le même trou que la ligne CADUC d'I2 — la traçabilité des engagements vaut pour les prédictions comme pour les portes »*. **Neuro re-signe, ou déclare s'abstenir, sur le design final APRÈS consolidation et AVANT gel.** |

**Règle de conflit reconduite** : si l'une des questions encore ouvertes du Directeur contredit
les positions 1 à 4, **l'argument structurel est requis ; l'argument de prudence est refusé
d'office** — *« coût des quatre tours »*.
