...
| D30 | **Le gel du pré-enregistrement se lève pour une erreur trouvable SANS la mesure, jamais pour une erreur qui n'apparaît qu'à la lumière des résultats.** Critère **exécutable, sans jugement** : *une donnée du run est-elle nécessaire pour voir l'erreur ?* Si **non**, la correction est licite sous **traçabilité complète** (ancienne valeur, nouvelle valeur, motif, auteur, section dédiée) et **l'autorisation elle-même s'inscrit dans le protocole** — le gel n'est jamais rompu en silence. Si **oui**, la prédiction gelée tient et sa falsification est un **résultat**. **Second alinéa (2026-08-26) — le critère de DIRECTION** : un changement post-gel **trouvable sans la mesure** mais qui n'est **pas** une correction d'erreur (une borne **valide mais lâche** que l'on resserre, une analyse **meilleure** que l'on substitue) est licite **si et seulement s'il rend la prédiction PLUS DURE à satisfaire** ; il est **interdit** s'il la rend plus facile. *« J'ai trouvé une meilleure analyse » est le chemin de bifurcation classique — mais un changement qui rend **sa propre hypothèse plus difficile à confirmer** est le contraire du geste que le gel interdit.* Le sous-critère est **exécutable** : on compare deux seuils. | **Le pré-enregistrement pose un dilemme sans bonne réponse naïve, et ce critère le tranche.** Sans lui, deux modes de mort symétriques : (i) **le gel sanctuarise les fautes d'arithmétique** — cycle « recouvrement des supports », 2026-08-26 : la porte `V-G` v1 était **insatisfiable par construction** (elle exigeait une reproduction bit-à-bit qu'un changement de dtype rendait impossible) et le protocole serait **mort bêtement** ; (ii) **« c'était une erreur » devient la sortie universelle après lecture des données** — c'est le mode que l'arrêt dur interdit. **Jurisprudence de cinq runs en une ligne** : « les mêmes `R` directions sur ℝ⁷⁶⁸ et ℝ¹⁵³⁶ » (0-133) se voyait **sur le papier** ⇒ corrigible sous gel ; la nulle fausse de `P1` en v3 ne se voyait **qu'avec les comptes du run** ⇒ jamais corrigible. Premier exercice : cycle « recouvrement », §15 — `A3` calculé avec une `G` distincte de celle du projet, et une colonne mal nommée, **trouvés en 9,1 s de CPU, avant toute mesure, sans une donnée de l'expérience**. |
| D31 | **Une nulle de permutation gèle sa STATISTIQUE COMPLÈTE — règle de signe, agrégation, traitement des atomes — au même titre que son schéma d'échange.** Quand **l'unité d'échangeabilité et la variable de stratification coïncident**, toute règle partielle possède des **cellules de dégénérescence**, et deux règles licites peuvent rendre des partitions **opposées sur les mêmes données**. Le banc D14-S **publie le cardinal des permutations effectives par cellule AVANT le gel** ; une cellule à cardinal dégénéré est déclarée **inatteignable pour sa classe d'avance**, jamais découverte après. | **Défaut 0-155, cycle « recouvrement », 2026-08-26.** Le §4.5 gelait la permutation intra-tige, `B = 10⁴`, `K_eff = 10` — **pas** la règle de signe des paires inter-tiges. Deux règles licites : `min` (implémentée) et `produit`. Les strates S3/S2 sont **100 % intra-tige**, S1/S0 **100 % inter-tige** — la variable de stratification **EST** la relation d'identité de l'unité d'échangeabilité. Résultat : **chaque règle est dégénérée exactement là où l'autre a de la puissance**, et les douze classes `C` **s'inversent** en passant de l'une à l'autre. Sous `produit` × intra-tige, `s_t·s_t = +1` : la permutation est **littéralement sans effet**, `ε* ≥ q₉₅(bootstrap) > IC_inf` est un **théorème**. Aucune des trois règles envisageables n'a de **primauté par dérivation** : la permutation exacte exigerait `O(a sous plac, b sous type)`, **jamais calculée**. La primaire décisionnelle du cycle est morte d'une clause que personne n'avait pensé à écrire. |
| D32 | **À ce locus (couche d'accroche, `G` seed 0), la nulle opératoire d'un recouvrement de supports n'est PAS `k²/D`** : ce sont **`p_sym(cos)`** — la borne d'identité de Cauchy-Schwarz réalisée — et **`top64(G·μ_global)`**. Toute mesure future de structure de support **publie, à côté de `O`** : (i) l'**excès `O − p_sym`**, puisque `O ≥ p_sym` est une **identité au poids probant nul** ; (ii) le **recouvrement avec `top64(G·μ_global)`**. **Faute de quoi elle re-publie l'état moyen sous un nouveau nom.** Le plancher chiffré de (ii) est **dû** (D20). | **Cycle « recouvrement », 2026-08-26.** `O` brut sort à 17-40 × `k²/D` — le chiffre le plus spectaculaire du run et **le moins informatif** : il mesure que le cosinus n'est pas nul, ce que `V-G` avait déjà établi. Contre `p_sym`, la marge réelle est de **1,12 à 1,24 ×**, et l'excès libre vaut **2,44 ± 0,34 indice** sous `aucun` contre **0,51 ± 0,38** sous retrait par type. Parallèlement, **`Core-G = 1/1`** sur les deux modèles à noyau non vide, et **sur les trois modèles les 12 indices les plus partagés sont tous dans `top64(G·μ_global)`** — `G` et `μ_global` **n'ont jamais vu le matériau**. Toute lecture représentationnelle d'un indice de `topk(G·h)` à ce locus est **fausse par provenance**, pas seulement invérifiable. |
| D33 | **Ce qui est vrai sous TOUTE complétion licite d'un texte gelé sous-spécifié est décidable sous le gel ; ce qui dépend du choix de complétion ne l'est pas.** Un énoncé **quantifié universellement sur la famille des complétions** ne dépend d'aucun choix non gelé : il se conclut. Un énoncé qui **sélectionne** une complétion — fût-ce par un critère structurel et calculable sans les données — reste soumis à **D30 alinéa 2** et tombe s'il rend la prédiction plus facile. | **Cycle « recouvrement », 2026-08-26 — le principe qui sauve un acquis sans ouvrir la porte de bifurcation.** La règle de signe de `ε*` n'étant pas gelée (D31), l'attribution entre `c-cent` et `ind_Δ` **dépend du choix** et reste **suspendue**. Mais *« aucune cellule, sous aucune règle licite, ne rend `c-mec` ni `c-anti` »* est vrai **sous toute complétion** : **`¬c-mec` et `¬c-anti` sont attribués**, et l'explication triviale — « le placebo apparié reproduit la chute » — est morte. Symétriquement, *« prendre chaque cellule sous la règle qui y a de la puissance »* **est** la règle hybride : elle sélectionne, elle fait passer les `c-cent` de 6 à 12, elle est **interdite**. La frontière n'est pas « avec ou sans données » — elle est **entre le quantificateur universel et le choix**. |
| D34 | **Une clause d'audit qui exige la publication d'une quantité de recouvrement exige la publication d'un COUPLE, jamais d'un nombre** : la quantité sous la cible **et** la même quantité sous une **clé nulle appariée mesurée dans le même run**, avec leur **différence**. *Une clause qui publie son propre contrôle apparié **produit toujours un plancher** (D26) ; une clause qui publie un nombre seul produit un **suspect innocenté** (D20).* **Application immédiate : D32 (ii) se lit désormais en couple** — `ρ_Base(cible)`, `ρ_Base(clé nulle appariée)`, et leur différence. | **Cycle « Étape A », cadrage du 2026-08-27 — la dette a été vue AVANT toute mesure.** D32 avait été gravée la veille **sans son plancher**, et `lab-math` a montré par quadrature que la quantité que D32 (ii) réclame est **prédictible avant mesure à partir du seul cosinus** `γ = cos(h, μ_global)` : `γ ≈ 0.75–0.89` ⇒ `ρ̂_Base ≈ 0.55–0.75`, contre un seuil de succès ratifié à **0.50**. Publiée seule, elle aurait été lue comme une propriété du support alors qu'elle est **imposée par la proximité cosinus** — et son antipode `< 0.25` exigerait `γ ≈ 0.5`, que les normes mesurées contredisent. **Le plancher n'était pas dans le nombre, il était dans le couple.** Rédaction de `lab-neuro`, retenue contre celle du Directeur : *« nommer la clé nulle » est déclaratif ; « publier le couple » est exécutable.* |
| D35 | **La couverture du banc D14-S s'étend au LECTEUR DE CLASSES et aux RÈGLES D'AFFICHAGE.** Toute partition (D18) inclut : **(i)** le **lecteur** qui transforme les mesures en classes — **un cas synthétique par classe**, où la classe rendue **et sa prose gravée** sont vérifiées **verbatim** ; **(ii)** les **règles d'affichage** — décimales, arrondis, agrégats de comptage — comme **clauses testées**. Toute quantité dont **deux affichages licites produisent deux lectures** publie ses deux affichages, ou gèle le sien. | **Étape A, 2026-09-04.** Le banc rendait `E = 0` sur 41 clauses, et **forcer le classificateur des 24 classes à toujours rendre `Σ-epuise` laissait `E = 0`** — trouvé par `lab-verifier` en **mutant la fonction de production**, la seule méthode qui pouvait le trouver. Et les **deux clauses gelées falsifiées** de ce cycle vivaient **dans la prose, aucune dans un chiffre**. Deux affichages ont failli sortir au public : un `IC_inf` de `+5.12e−06` qui **s'affiche `0.0000`** à 4 décimales (et se serait lu « ordre établi »), et un compte publié **`2`** qui **n'est dérivable sous aucune lecture cohérente** — il vaut **1** sous la lecture décisionnelle, **3** sous « toute lecture publiée », et n'était exact que **par compensation de deux erreurs de signe opposé**. Formule de `lab-math` : *« le résidu d'erreur se loge dans ce qu'aucun test n'exerce : les lectures en prose des classes, et le code qui les affiche. »* |
| D36 | **Toute classe d'EXTINCTION porte une CONDITION DE PUISSANCE gravée d'avance.** Une classe qui conclut *« il n'y a pas d'anomalie à expliquer »* n'est licite que jointe à **la frontière chiffrée au-delà de laquelle l'anomalie aurait été détectée si elle existait**, et à **la résolution effective publiée**. Faute de marge suffisante, la cellule rend **`-ind`, cause nommée = puissance** — jamais `-mort`. ***Une extinction sans condition de puissance est un innocentement*** (D20, transposé aux extinctions). **Corollaire** : la **portée** d'une classe d'extinction — globale ou par cellule — se grave **avec elle**. | **Étape A, 2026-09-04.** Les trois `Σ-mort` tombent **toutes** sur la plus petite cellule de la grille, `N_eff = 19,8 / 26,6 / 27,6`, IC larges de 0,34 à 0,41 — et **deux ont un écart ponctuel à 0.5 PLUS GRAND que les cellules-titres** (+0,158 et +0,166). Elles ne « survivent pas » **uniquement faute de puissance** : la clause gelée, appliquée verbatim, **conclut à l'absence d'effet depuis une absence de puissance** — exactement ce que D28 interdit. Et sa **portée** était fausse : elle annonçait l'extinction du **brin entier** (« `N-queue` sans objet »), or la classe se réalise **par cellule** et **coexiste avec 18 `Σ-epuise`** — *elle traitait comme un interrupteur ce qui est une partition par bucket*. La condition est **exécutable** : le §14.5 du même protocole la chiffrait déjà (`ρ_ic ≥ 0.26–0.38`). |
| D37 | **Le gel ne produit pas des protocoles sans erreur : il produit des erreurs DATÉES, COMPTABLES et NON RÉPARABLES EN SILENCE.** **Corollaire exécutable** : une **falsification de clause gelée**, détectée avant ou pendant la mesure, **se signale sans se corriger**, **se tranche au dépouillement comme un résultat de plein rang**, et **s'inscrit au registre avec son adresse** — la clause **reste opérante sur sa condition**, seule sa **prédiction annexe** est fausse. | **Étape A, 2026-09-04 — deux occurrences dans un seul protocole.** (i) Le §4.5 annonçait le vivier vide *« attendu possible en S3 »* ; l'énumération donne **S3 = 1 sur 60/60 paires — jamais vide** — et **S2 = 0 sur 90/90 — vide certain**. `Δ-sansobjet` s'est déclenchée sur **S2**, six cellules : **la classe est correcte, son adresse était fausse.** (ii) La lecture de `Σ-mort` annonçait une extinction globale ; elle se réalise par cellule (voir **D36**). **Aucune des deux n'a été réparée**, et les deux se lisent comme des résultats. *La pratique méritait le rang de décision pour ne plus dépendre de la vertu du rédacteur suivant.* |
| D38 | **Une source EXTERNE se cite par COPIE ARCHIVÉE ET HASHÉE, jamais par URL vivante — ni par une vérification faite en conversation.** Toute référence hors du dépôt entre au registre des chiffres avec : la **copie du document** dans un répertoire versionné, son **SHA256**, sa **date de consultation**, son **URL d'origine** et l'**extrait cité**. Étiquette de provenance : **`externe-archivé`** — troisième valeur de D14-R, qui n'en avait que deux. **Un chiffre externe sans copie archivée est un `ORPHELIN`**, au même titre qu'un chiffre sans source. | **Cycle de rédaction, 2026-09-04.** Le coordinateur avait vérifié une référence industrielle par recherche web — quatre sources concordantes, chiffres précis — et l'avait transmise comme acquise. `lab-neuro` l'a récusée : *« je ne peux vérifier ni son existence ni sa table ; tant qu'un artefact lisible n'est pas au manifeste, c'est une **invention potentielle** »*, et surtout : ***« un interdit ne remplace pas une provenance »*** — l'interdit rédigé pour cadrer la portée de cette référence **cadrait** sans **sourcer**. **La vérification était réelle et l'objection tient quand même** : *« vérifié en conversation »* n'est pas une adresse qu'un lecteur ouvre, et **une URL vivante ne l'est pas davantage** — elle change, elle disparaît, elle n'a pas de hash. C'est D14-R appliquée au coordinateur lui-même. Corollaire gravé au même cycle : **une adresse non ouvrable est un défaut, pas une imprécision** — le piège du répertoire exclu des outils de recherche s'était alors déclenché **trois fois**. |
| D39 | **La clôture d'un instrument passe par son LIVRABLE, et par un outil qui n'est pas le sien.** Deux alinéas, chacun exécutable. **(1) Aucune garantie ne se déclare depuis le source.** Une garantie protégeant un livrable (non-écrasement, hash, format) s'établit **en exécutant le point d'entrée** et en observant **l'état du fichier** — jamais par une assertion portant sur le texte du programme (`assert '…' in src`). **(2) Un artefact publié se vérifie par l'outil standard EXTERNE, jamais par son propre lecteur.** | **Cycle de rédaction, phase instrument, 2026-09-05.** (1) `0-261` — le garde-fou du Builder contre `0-243` était **un grep sur deux chaînes littérales**, et son test **gelait la liste fausse dans une assertion** ; au tour 4 le mode **s'est reproduit sur son propre correctif** : **aucun test n'appelait `main()`**, et l'insertion d'**une seule ligne** en bac à sable a ressuscité `0-254` intégralement — l'artefact publié écrasé par un rapport `force_pass=True` 32/32 `PASS`, **la console affichant « l'artefact publié n'est PAS écrit » pendant qu'elle le détruit**, les deux gardes passant. (2) À l'octet : `report/SHA256SUMS` en **CRLF**, `sha256sum -c` échouant **55/55** tandis que `verifier_sommes()` rendait OK, son `splitlines()` absorbant le `\r` — ***le seul outil capable de vérifier nos sommes était celui qui les avait écrites*** — et `core.autocrlf = input` sans `.gitattributes` rendant les **55 hashes faux dès le commit**. **Corollaire de portée générale : la porte la moins chère est celle d'un ÉTAGE QUE LE CONSTRUCTEUR N'A PAS REGARDÉ.** Sixième occurrence consignée d'une porte gagnante sans données (`V-slot`, `V-ident`, puis les quatre tours du 2026-09-05) — et **première où l'étage gagnant est le FORMAT DU LIVRABLE, non la logique**. |
